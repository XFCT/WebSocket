...
NSString * const kNeedPayOrderNotification               = @"kNeedPayOrderNotification";
...
+(SocketRocketTool *)instance{
    ...
    dispatch_once(&predicate, ^{
        Instance = [[SocketRocketTool alloc] init];
    });
    return Instance;
}
...
- (void)sendData:(id)data {
    ...
    WeakSelf;
    dispatch_queue_t queue =  dispatch_queue_create("XFC", NULL);
...
- (void)init_heartBeat
{
    dispatch_main_async_safe(^{
        [self destory_heartBeat];
        //心跳设置为 10S，NAT超时一般为5分钟
        _heartBeat = [NSTimer timerWithTimeInterval:10 target:self selector:@selector(sentheart) userInfo:nil repeats:YES];
...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    ...
    if (webSocket == self.socket) {
        ...
        [self SRWebSocketClose];
        [[NSNotificationCenter defaultCenter] postNotificationName:kWebSocketDidCloseNotification object:nil];
    }
}
